...
/**
 * Packager pour une structure TLV en Ascii, les données sont de l'ASCII, le type est de l'hexadécimal en ascii sur 2 octets, et la longueur est de l'ascii sur 2
 * octets.
 * 
 * @author keyser_pa, dgrandemange
 * 
 */
public class ISOAsciiHexTLVSubFieldPackager extends ISOAbstractTLVSubFieldPackager
{
	@Override
	protected int unpack(ISOComponent m, byte b[], int consumed) throws ISOException
	{
		String hexString = new String(b, consumed, 2);		
		
		int fieldNo = -1;
		try
		{
			fieldNo = Integer.parseInt(hexString, 16);
		}
	...
	protected byte[] fieldPack(ISOField field) throws ISOException
	{
		...
		String subField = ISOUtil.padleft(String.format("%H", (Integer) field.getKey()), 2, '0');
